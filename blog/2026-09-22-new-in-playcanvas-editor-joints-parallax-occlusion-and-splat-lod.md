...
- **Client-side font generation** (since 2.30.0). MSDF atlas generation moved out of the server-side conversion pipeline and into the browser, and the descriptor and atlas pages now land as separate assets you can inspect, repoint or swap for your own. The **Characters** field is editable, so you can widen the character set later and regenerate.
