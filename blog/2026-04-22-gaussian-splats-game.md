---
title: "Building an FPS game Inside a Gaussian Splat"
...
tags: [gaussian-splats, supersplat, playcanvas, tutorial]
---
...
`splat-transform` is PlayCanvas's open-source CLI for slicing, dicing and converting splats. The flag you want is `-K` / `--collision-mesh`. It voxelizes the splat, flood-fills the navigable interior from a seed position, and writes out a watertight `.collision.glb` that you can import straight into PlayCanvas as a `mesh` collider.
...
## 💡 Step 2 — Baking Lightness Probes Into the Splat
...
👉 **Call to action:** install the [PlayCanvas VS Code extension](https://marketplace.visualstudio.com/items?itemName=playcanvas.playcanvas). If you live in VS Code or Cursor, it turns PlayCanvas into a normal dev environment.

## 🔄 Step 4 — Versioning With PlayCanvas + GitHub

The next pain point is "what did I change yesterday and how do I roll back?". PlayCanvas ships a first-party [**version control**](https://developer.playcanvas.com/user-manual/editor/version-control/). You can also use GitHub at the root of your locally synced PlayCanvas project (by VS Code extension).
...
👉 **Call to action:** link a GitHub repo to your PlayCanvas project before you start. You'll thank yourself the first time an agent commits a bad refactor at 1 AM.

## 🧭 Step 5 — Generating a Navmesh from the Collision Mesh
...
The runtime side is already open — `NavMeshWrapper` in [`Scripts/npc-ai.js`](https://playcanv.as/p/qxGSuzYq/) wraps Recast into a tidy `findPath / getClosestPoint / moveAlongSurface / getRandomPoint` API with a world-space offset for coordinate conversion. It's about 200 lines and has zero PlayCanvas deps — you can lift it straight into another engine.

## 🧠 Step 6 — NPC Behavior: Navmesh + Behavior Trees + Personalities
...
`npc-ai.js` exposes four primitives:
...
👉 **Call to action:** if you've been nervous about behavior trees, read [Chris Simpson's primer](https://www.gamedeveloper.com/programming/behavior-trees-for-ai-how-they-work) and then copy my four-function implementation. It's 20 lines of code and it unlocks production-grade AI.
...
SuperSplat, SplatTransform, recast-navigation and the PlayCanvas Engine are all open source. The scripts in this demo — `probes.js`, `npc-ai.js`, `npc-controller.js`, the navmesh wrapper — are free for you to copy, fork, or improve. If you take them somewhere cool, I want to see it.
